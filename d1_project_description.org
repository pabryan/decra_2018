...
2. Prove the multiplicity one conjecture for Curve Shortening of Networks,
3. Prove a Grayson theorem for Homogeneous Curve Shortening Flow.

*** Background

Isoperimetric comparisons also play an important role in the study of curvature flows. In the papers \cite{MR2729306,MR2843240,MR2794630,Bryan}, isoperimetric estimates under curvature flows were used to obtain curvature bounds and direct proofs of convergence results. Such comparisons are very powerful and could prove extrelemly effective in the analysis of higher dimensional Ricci flow, Yamabe flow, the Cross Curvature Flow (see section on Cross Curvature Flow) and the Curve Shortening Flow of networks.

*** Comparison theorems for the isoperimetric profile.

The isoperimetric profile of a Riemannian manifold was introduced in the 1980's \cite{MR875084,MR999971} to study isoperimetric inequalities on Riemannian manifolds. By definition, for a given real number $x$, the isoperimetric profile $I(x)$ is the infimum of the boundary area of all domains with volume equal to $x$:
\[
I(x) = \inf\{\operatorname{Area} (\bdry{\Omega}) : \operatorname{Vol}(\Omega) = x\}
\]
For instance, the isoperimetric profile of the plane is $I(x) = \sqrt{4\pi x}$ which follows immediately from the classical isoperimetric inequality $L^2 \geq 4\pi A$ with equality attained by round circles.
...
\]
for a smooth, on-parameter family of immersed curves $\gamma_t \subset \RR^2$ in the plane where $\kappa$ is the geodesic curvature with respect to a smooth choice of unit normal, $\nu$. The most famous result regarding this equation is that when $\gamma$ is a closed, embedded curve it collapses to a point in finite time, smoothly converging to a circle after rescalining to keep the total length fixed. Initially the result was proven for strictly convex curves \cite{MR840401} and then later it was shown that arbitrary closed, embedded curves become convex \cite{MR906392} and then the result for convex curves applies. In \cite{MR2794630}, with Prof. Ben Andrews I proved a distance comparison principle leading to a curvature bound and subsequent direct proof of this result in the general case without convexity assumptions.

A related flow is the flow,
\[
\partial_t \gamma = -\kappa\abs{\kappa}^{\alpha-1} \nu.
\]
This flow has greater non-linearity than the Curve Shortening Flow, and owing to the absolute value, has non-smooth speed for non-convex curves. In the case of convex curves, when $\alpha = 1/3$ the flow is known as the Affine Curve Shortening Flow and strictly convex initial curves collapse to a point in finite time, smooth converging to ellipses after rescaling \cite{MR1255274}. More generally, it was shown \cite{MR1660843} that for $\alpha > 1/3$, convex curves collapse to a point in finite time, smoothly converging to a circle after rescaling.

The aim here is to extend these results from convex curves to arbitrary emedded curves. To achieve this end, first we define approximating flows,
\[
...
Comparison of chord-arc profiles for $\alpha \in (1/3, 1]$: For such flows, in order to couple time and spatial variations of the chord-arc profile, the speed, $f$ of the flow must be a so-called /operator monotone/ function, which means upon applying the spectral theory we have for two matrices $A$ and $B$,
\[
A \leq B \Rightarrow f(A) \leq f(B).
\]
This restricts us to consider $\alpha \in (0,1]$, in which case the following differential inequality for the chord-arc profile holds,
\[
\pd{t}Z \geq 2 \frac{2^{\alpha+1}}{\left(\sqrt{1 - (Z')^2}\right)^{\alpha-1}} f(Z'') - \avg{f(\curvecurv) \curvecurv} \left(Z - Z'\ell\right) - Z' \int_{p_0}^{q_0} f(\curvecurv)\curvecurv ds
\]
where $f(y) = y\abs{y}^{\alpha-1}$ is the speed of the flow, $\avg{\cdot}$ denotes the mean value over the curve $\gamma$ and $(p_0, q_0)$ are points of given intrinsic length $\ell(p_0, q_0)$ achieving the infimum of extrinsic distance $d(p_0, q_0)$.

Given the critical exponent $\alpha=1/3$ discussed above, it's expected that the curvature estimates obtained using the methods in \cite{MR2794630} should hold for $\alpha \in (1/3, 1]$. Supporting evidence for this conjecture is that the differential inequality is linearly stable around the round circle.

For the case $\alpha \geq 1$, the Harnack inquality (see also the section on estimates below) was used in \cite{MR1094458} to show that arbitrary metrics on $\sphere^2$ become positively curved in finite time under the Ricci flow. Then the results of \cite{MR954419} show convergence to the constant curvature metric. The aim here is to show similar results hold for the approximating flows defined above.

*** Curve Shortening of Networks (joint with Prof. Mariel \saez)

The distance comparison argument in \cite{MR2794630} applies also to the Curve Shortening Flow of networks that has received considerable attention in recent years \cite{MR2394409,MR2075985,MR2763716}. In this context, a network is an emedded graph in the plane where the two vertices of each edge either lie on the boundary, $\bdry{\Omega}$ of a convex body with interior (or on the circle at infinity) or meets precisely two other edges at a vertex with equal angles of $2\pi/3$ (/triple condition/). The edges are supposed further to be otherwise disjoint. See figure \ref{fg:network}.
\begin{figure}[htb]
\centering
...
The aim here is to use construct suitable comparison functions in order to obtain a curvature bound similar to \cite{MR2794630}. The network flow has a number of differences to the smooth flow requiring further analysis. Two important differences are:

1. The asymptotics of the chord arc profile have a different lower order constant and include a quadratic term not present in the smooth case,
2. The triple points introduce new terms into the viscosity equation not present in the smooth case.

These two points then require modification of the arguments used in the smooth case. However, though not explicitly articulated in \cite{MR2794630}, there is a powerful procedure for finding suitable comparison functions: make a similarity substitution using the profile of the self-similar solution in the viscosity equation. In the smooth case, this is the circle but in the network case there are multiple self-simlar solutions and the comparison must take this into account. On the other hand, the correct asymptotics and additional terms arising from the triple points are present in the self-similar solution so the points above are adequaltely dealt with.

The aim then is to complete this program and prove the major current oustanding conjecture, that the only singularities possible are of multiplicity one. This follows essentially from a chord-arc comparison which shows that the whenever and edge has positive length, all other points remain at positive extrinsic distance hence the only collapse possible is when an edge collapses to zero length. Note that there are heuristic examples giving strong evidence that such behaviour does indeed occur.
...
** The Cross Curvature Flow and hyperbolic manifolds (joint with Dr. Mohammad Ivaki and Dr. Julian Scheuer)

*** Aims

1. Prove that the XCF deforms strictly negatively curved metrics to constant curvature metrics and hence path connectedness of the moduli space of negatively curved metrics on hyperbolic manifolds,
2. Prove isoperimetric comparisons for the XCF,
3. Generalise the XCF to higher dimensions.

*** Background

In \cite{MR2602839} it is noted that in Thurston's geometrisation program, hyperbolic manifolds are the largest class of prime manifolds. It's not known whether this space is contractible, or even connected. More generally, one may ask similar questions about the classes of pinched metrics.

The Cross Curvature Flow (XCF) was introduced in \cite{MR2055396} in the hopes of deforming negatively curved metrics $g_0$ on a compact three-manifold $M$ to constant curvature metrics in a similar manner to the seminal work on Ricci flow for positively curved metrics \cite{MR664497}. Such a result would show contractibility of the space of negatively curved metrics on a hyperbolic manifold. Very little apart from short-time existence and uniqueness \cite{MR2207496} and some results in special situations such as homogeneous spaces and tori \cite{MR2222245,MR2448593,MR2602839, MR2426751,MR2407107, MR2653711, MR2601352} is known about solutions of the cross-curvature flow, but such results support the conjecture that XCF deforms negatively curved metrics to constant curvature metrics.

Let
\[
E = \operatorname{Ric} - \tfrac{1}{2} \operatorname{R} g
\]
denote the Einstein tensor of a Riemannian manifold $(M, g)$. We define the endomorphism $\mathcal{E}$ of the tangent bundle $TM$ by,
\[
E(X, Y) = g(\mathcal{E}(X), Y).
\]
This enables us to define the Einstein-Ricci tensor,
\[
...
\]
typically written in coordinates (depending on curvature tensor conventions),
\[
\partial_t g_{ij} = -2 E^{kl} R_{kijl}
\]
where $E^{ij} = g^{ik}g^{jl}E_{kl}$ is the raising of $E$ by the metric $g$. In three dimensions, whenever $(M, g)$ has strictly negative sectional curvature, $E$ is positive definite and hence defines a metric.
...
The techniques just described, particulary the Harnack inequality also relate closely to solitons - self similar solutions of the flow $g(t) = e^{u(t)} \phi_t^{\ast} g_0$ moving by the scaling factor $e^{u(t)}$ and by the one-parameter family of diffemorophisms. These techniques should yield convergence to a soliton, and the next step is to classify such solitons as constant negative curvature metrics. In the locally-embedible case, such a result was proven in Minkowksi space \cite{MR3344442} and recently in Euclidean space \cite{2016arXiv160905487C,2016arXiv161008933B} giving strong indication that the result for the XCF may be obtained similarly.
...
The aim here is to develop a theory of short-time existence and uniqueness as well as all the basic evolution equations necessary for the analysis of these flows in higher dimensions. In this situation, strictly negative curvature does not correspond to positive-definiteness of $E$, again diagonalising $\operatorname{ric}$ shows the conditions of strictly negative curvature operator and vanishing Weyl tensor are related to the postive definiteness of $E$ and hence are hence natural conditions to impose in the initial study which likely will lead to similar results as those in the three dimensional case.

** Singularities, symmetries and ancient solutions of fully non-linear geometric evolution equations (joint with Dr. Mohammad Ivaki and Dr. Juliean Scheuer)
...
In \cite{2016arXiv160401694B} my collaborators and I proved that effectively any convex, ancient solution of a parabolic, geometric flow must be a family of shrinking geodesic spheres emenating from an equator at $t=-\infty$. This is in stark constrast to the Euclidean space situation when many such solutions exist. The methods employed relied heavily on the geometry of the sphere and to generalise these results to Riemannian backgrounds, different methods must be adopted.

With this end in mind, given an unstable minimal surface $M_0$, we linearise the curvature flow and apply stability analysis such as in \cite{MR1485470,2016arXiv160104986H}. The theory of maximal regularity \cite{MR3012216} implies that the space of ancient solutions converging backwards to $M_0$ as $t\to-\infty$ is a finite dimensional linear sub-manifold of the space of smooth functions $M_0 \to \RR$ with dimension equal to the Morse index of $M_0$. Further analysis should show that there is unique mean convex ancient solution corresponding to the first eigenvalue which is simple (the eigenspace is one-dimensional). In this project I will complete making this program rigourous and by applying compactness results show that arbitrary non-singular ancient solutions fit into this framework.

As a striking example of possible applications, we see that there is a five-dimensional family of ancient solutions emenating from a Clifford Torus in $\S^3$. The motion of this family is, to first order, by the canonical family of conformal transformations used in the resolution of the Wilmore conjecture \cite{MR3152944}. This suggests a hitherto unknown connection between the conformal geometry of the sphere and the Mean Curvature flow with applications to Wilmore surfaces.

In general backgrounds, similar results hold and two points are of note:
1. There is a unique mean convex ancient solution emenating from an unstable minimal surface $M_0$,
2. Regularity theory of mean convex mean curvature flow has undergone significant development recently \cite{2013arXiv1304.0926H,MR3570481}
Using these these two results, I will extend the geometric inequalities of \cite{MR1650335} to the unique mean convex mean curvature flow (which may undergo signularities previously hampering progress with this method) to obtain area control for $M_0$ in terms of the value of the first eigenvalue.

For flows other than the mean curvature flow, one can encounter degenerecies and singularities in the linearisation around a minimal surface. The results in \cite{2016arXiv160401694B} show that nevertheless classification results are possible. To progress then I will develop approximating techniques, e.g. by approximating the flow with speed $H^p$ by the flow with speed $H (H^2 + \epsilon^2)^{\tfrac{p-1}{2}}$ (similar to the approximating flows for the $\kappa^p$ homogeneous curvature flows), obtain a-prior estimates and classification, then pass to the limit $\epsilon \to 0$ to conclude the classification for the original flow.

** Estimates for fully non-linear geometric evolution equations (joint with Dr. Mohammad Ivaki and Dr. Julian Scheuer)

*** Aims

1. Obtain and extend Harnack estimates for curvature flows,
2. Obtain modulus of continuity estimates for curvature flows.
