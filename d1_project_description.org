...
\]
which are smooth, strictly parabolic flows. The next step is to obtain a priori estimates via two approaches: one for $\alpha \in (1/3, 1]$ and one for $\alpha \geq 1$. Both methods are applicable to the critical case of the Curve Shortening Flow, $\alpha = 1$, the first method being the distance comparison principle in \cite\cite{MR2794630}.
...
A distance comparison based on \cite{MR1656553} was used in the special situation of convex, lens shaped networks. More generally, Prof. Mariel \saez I have shown that the comparison principle for the chord-arc profile developed in \cite{MR2794630} holds for networks after suitable modification to deal with the nodes. Namely the viscosity equation for the chord-arc profile is
\[
Z''
\]
which is identical to the smooth case apart from the additional $\Delta v_i$ terms.
