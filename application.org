...
The chord arc-profile is defined by,
\[
Z(x) = \frac{2\pi}{L} \inf \{d(p, q) : \ell(p,q) = \tfrac{L}{2\pi} x\}
\]
where $d$ denotes the Euclidean distance between $p$ and $p$, $\ell$ the length along the curve and $L$ the total length of the curve.
...
\]
which are smooth, strictly parabolic flows. The next step is to obtain a priori estimates via two approaches:

1. Comparison of chord-arc profiles: For such flows, in order to couple time and spatial variations of the chord-arc profile, the speed, $f$ of the flow must be a so-called /operator monotone/ function, which means upon applying the spectral theory we have for two matrices $A$ and $B$,
\[
...
2. For the case $\alpha \geq 1$, the Harnack inquality (see also the section on estimates below) was used in \cite{MR1094458} to show that arbitrary metrics on $\sphere^2$ become positively curved in finite time under the Ricci flow. Then the results of \cite{MR954419} show convergence to the constant curvature metric. The aim here is to show similar results hold for the approximating flows defined above.
...
In the locally isometrically embedible situation we obtained the Harnack inequality \cite{BIS4},
\[
\partial_t \sqrt{{\det} E} - \frac{1}{\sqrt{{\det}E}} \abs{\nabla \sqrt{{\det}E}}_E^2  + \frac{3}{4 t} \sqrt{{\det}E} \geq 0
\]
where $\abs{\cdot}_E$ refers to the norm with respect to the Einstein tensor $E$. Along with $C^0$ estimates, this should give an alternate proof of the conjecture. In fact, all that is required is a Harnack inequality and not the isometrically embedible assumption. Proving the Harnack involves great computational complexity in the general case, though early computations show it is possible to obtain a Harnack under the assumption, $\nabla V$ is totally symmetric. In the locally embedible case this follows from the Codazzi equation, but does not necessarily imply local embedibility. These initial results are thus already more general than those in \cite{MR3344442}.
